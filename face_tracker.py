...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python face_tracker.py <robot_ip> [camera_url]\n"
            "Find <robot_ip> in the Arduino Serial Monitor after flashing "
            "the firmware (it prints 'Robot IP address: x.x.x.x')."
        )
        sys.exit(1)

    robot_host = sys.argv[1]
    camera_url = sys.argv[2] if len(sys.argv) > 2 else "http://admin:12345678@192.168.1.3:8081/video"

    bot = Robot(robot_host)
    tracker = FaceTracker(bot, camera_url=camera_url)
    print("Starting face tracking. Press ESC in the preview window to stop.")
    tracker.run()
